...
const start = async () => {
  ...
  const render = () => {
    ...
    app.innerHTML = `
      ...
      <main class="wiki-shell"><aside class="wiki-toc"><p>本页目录</p>${toc || '<span>本页没有目录</span>'}</aside><article class="wiki-article"><div class="article-heading"><p class="article-eyebrow">来福Simulation Wiki</p><h1>${escapeHtml(page.title)}</h1></div><div class="article-body">${page.html}</div><footer class="article-license">本页内容迁移自来福Simulation Wiki，除另有注明外，依照 <a href="https://creativecommons.org/licenses/by-sa/3.0/deed.zh-hans" target="_blank" rel="noreferrer">CC BY-SA</a> 许可协议提供。</footer></article><aside class="wiki-sidebar"><h2>Wiki 条目</h2><p>共 ${pages.length} 篇迁移文章</p><a href="${routeFor(home.slug)}">返回首页</a><h3>最近条目</h3>${pages.slice(-8).reverse().map((item) => `<a href="${routeFor(item.slug)}">${escapeHtml(item.title)}</a>`).join('')}</aside></main>
