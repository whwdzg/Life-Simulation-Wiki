...
const start = async () => {
  ...
  const render = async () => {
    ...
    app.innerHTML = `
      <header class="wiki-header"><a class="wiki-brand" href="${routeFor(home.slug)}"><img class="wiki-brand-logo" src="${theme.icon}" alt="来福Simulation 标志"><span>来福Simulation Wiki（镜像站）</span></a><nav><a href="${routeFor(home.slug)}">首页</a><button class="all-pages-button" type="button" aria-expanded="false">全部页面</button><a href="${routeFor(encodeURIComponent('Special:SpecialPages'))}">特殊页面</a><a href="${routeFor(encodeURIComponent('Special:AllPages'))}">讨论</a><a href="${routeFor(encodeURIComponent('Special:AllMaps'))}">地图</a></nav><button class="search-toggle" type="button" aria-label="搜索">�</button><form class="wiki-search" role="search"><label class="sr-only" for="wiki-search-input">搜索 Wiki</label><input id="wiki-search-input" type="search" placeholder="搜索这个镜像站"><button type="submit" aria-label="搜索">⌕</button></form><button class="mobile-toc-button" type="button" aria-expanded="false">目录</button></header>
      ...
      <main class="wiki-shell"><article class="wiki-article"><div class="article-heading"><p class="article-eyebrow">来福Simulation Wiki 镜像站</p><h1>${escapeHtml(page.title)}</h1></div><div class="article-body">${articleHtml}</div>${isHome ? syncPanel(sync) + deployPanel(deploy) : ''}${revisionPanel(page)}<footer class="article-license">本页为<a href="https://lifesimulation.fandom.com/zh/wiki/%E6%9D%A5%E7%A6%8FSimulation" target="_blank" rel="noreferrer">来福Simulation Wiki</a> 的静态镜像，除另有注明外，依照 <a href="https://creativecommons.org/licenses/by-sa/3.0/deed.zh-hans" target="_blank" rel="noreferrer">CC BY-SA</a> 许可协议提供。源代码以GPL v3许可协议开源。本站不提供编辑内容功能，也无法完全替代原页面，仅供静态页面浏览。</footer></article><aside class="wiki-sidebar"><section class="wiki-outline"><h2>本页目录</h2><nav>${outline}</nav></section>${quoteHtml ? `<section class="home-quotes">${quoteHtml}</section>` : ''}<section class="wiki-pages"><h2>Wiki 镜像条目</h2><p>共 ${pages.length} 篇迁移文章</p><a href="${routeFor(home.slug)}">返回镜像首页</a><h3>最近条目</h3>${pages.slice(-8).reverse().map((item) => `<a href="${routeFor(item.slug)}">${escapeHtml(item.title)}</a>`).join('')}</section><section class="wiki-special-links"><h2>快速入口</h2><a href="${routeFor(encodeURIComponent('Special:SpecialPages'))}">特殊页面</a><a href="${routeFor(encodeURIComponent('Special:AllPages'))}">讨论区</a><a href="${routeFor(encodeURIComponent('Special:AllMaps'))}">地图</a><a href="https://lifesimulation.fandom.com/zh/wiki/Special:SpecialPages" target="_blank" rel="noreferrer">原站特殊页面</a></section></aside></main>
